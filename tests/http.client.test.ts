...
import type { TMDBOptions } from "../src/client";
import { TMDBError } from "../src/client";
...
describe("HttpClient", () => {
  ...
  it("throws TMDBError on non-2xx with parsed message/code", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValue(
      jsonResponse({ status_message: "Invalid API key: You must be granted a valid key.", status_code: 7 }, { status: 401 })
    );
